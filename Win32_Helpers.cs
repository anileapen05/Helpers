...
namespace Helpers_1_0
{
    class Win32_Helpers
    {
        ...
        static public extern HKL ActivateKeyboardLayout(HKL hkl, uint Flags);

        [DllImport("user32.dll")]
        static public extern int ToUnicodeEx(uint wVirtKey, uint wScanCode, byte[]
           lpKeyState, [Out, MarshalAs(UnmanagedType.LPWStr)] StringBuilder pwszBuff,
           int cchBuff, uint wFlags, IntPtr dwhkl);

        [DllImport("user32.dll")]
        static public extern int GetKeyNameText(int lParam, [Out] StringBuilder lpString, int nSize);

        [DllImport("user32.dll")]
        static public extern IntPtr LoadKeyboardLayout(string pwszKLID, uint Flags);

        [DllImport("user32.dll")]
        static public extern bool GetKeyboardLayoutName([Out] StringBuilder pwszKLID);

        [DllImport("user32.dll")]
        static public extern bool UnloadKeyboardLayout(IntPtr hkl);

        [DllImport("user32.dll")]
        static public extern IntPtr GetKeyboardLayout(uint idThread);
